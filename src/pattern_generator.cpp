...
void PatternGenerator::setMode(Mode newMode) {
    mode_ = newMode;
    if (mode_ == Maximal127) {
...
void PatternGenerator::setSeed(uint8_t newSeed) {
...
void PatternGenerator::clock(float change, float chaosSignal) {
    if (mode_ == Maximal127) {
        ...
        const uint8_t currentState = nonZero127Seed(state_);
        const uint8_t feedback = static_cast<uint8_t>(
            (currentState ^ (currentState >> 1u)) & 1u);
        state_ = static_cast<uint8_t>(
            ((currentState >> 1u) | (feedback << 6u)) & 0x7fu);
        return;
    }
